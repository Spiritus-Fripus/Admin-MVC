<div class="main-container-center-column">
                ...
                <tr class="card-tr">
                    ...
                    <td data-label="Actions">
                        <div class="button-crud">
                            <a href="/archiveFormation?formation_id=<?= $formation['formation_id'] ?>"
                                class="delete">
                            ...
                            <a href="/modifyFormation?formation_id=<?= $formation['formation_id'] ?>"
                                class="modify">
                                <svg xmlns="http://www.w3.org/2000/svg" height="20px" viewBox="0 -960 960 960"
                                    width="20px"
                                    fill="#283e58">
    ...
    <div class="paging">
        ...
        <?php for ($i = 1; $i <= $totalPages; $i++): ?>
            <a href="/formation?page=<?= $i ?>&search=<?= urlencode($filters['search']) ?>&sort-by=<?= urlencode($filters['orderBy']) ?>&sort-direction=<?= urlencode($filters['direction']) ?>" <?= $i == $page ? 'class="active"' : '' ?>><?= $i ?></a>
        <?php endfor; ?>

        <?php if ($page < $totalPages): ?>
            <a href="/formation?page=<?= $page + 1 ?>&search=<?= urlencode($filters['search']) ?>&sort-by=<?= urlencode($filters['orderBy']) ?>&sort-direction=<?= urlencode($filters['direction']) ?>">Suivant</a>
        <?php endif; ?>
    </div>
</div>
